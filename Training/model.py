...
class GeneratorNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(nn.Conv3d(6, 32, kernel_size=3, padding=1, bias=True), nn.BatchNorm3d(32),)
        ...
        self.deConv4 = nn.Sequential(nn.LeakyReLU(inplace=True), nn.Conv3d(32, 6, kernel_size=3, padding=1),)
...
class WGAN:
        ...
        for epoch in range(0, self.epochs):
            ...
            for batch_index, batch in enumerate(self.dataloader):
                ...
                for iter_i in range(self.d_iter):
                    ...
                    print(
                        "\tVGG_MSE - lr: %.10f, Level: %d, Epoch: %d, bath_index: %d, iter: %d, G-Loss: "
                        % (self.lr, self.level, epoch, batch_index, iter_i),
                        loss,
                    )
    ...
    def _train_discriminator(self, free_img, noised_img, train=True):
        ...
        gradient_penalty = self._calc_gradient_penalty(real_img.data, fake_img.data)
        ...
        return (
            d_loss.data.item(),
            torch.mean(-real_validity).cpu().item(),
    ...
    def _calc_gradient_penalty(self, free_img, gen_img):
        ...
        alpha = alpha.expand(batch_size, free_img.nelement() // batch_size).contiguous().view(free_img.size()).float()
        if self.gpu:
            alpha = alpha.cuda()
    ...
    def test(self, epoch):
        ...
        print(
            "%s Epoch： %d lr：%.10f Test Loss：g-loss: %.4f vgg-loss: %.4f mse-loss： %.4f d_loss: %.4f"
    ...
    def compute_quality(self):
        ...
        for i in range(101, 111):
            ...
            noised_nii = nib.load("./data/dataset/noise_%d/%d.nii" % (self.level, i))
            ...
            denoised_img = merge_test_img(self.denoising(patchs), row, col).astype(np.int16)
            ...
            ssim1 += compare_ssim(free_img, noised_img, data_range=4096, multichannel=True)
            ssim2 += compare_ssim(free_img, denoised_img, data_range=4096, multichannel=True)
            ...
            _ssim1 += compare_ssim(free_img, noised_img, data_range=max, multichannel=True)
            _ssim2 += compare_ssim(free_img, denoised_img, data_range=max, multichannel=True)
    ...
    def save_model(self):
        ...
        torch.save(self.generator.state_dict(), self.save_dir + "G_" + self.v + ".pkl")
        torch.save(self.discriminator.state_dict(), self.save_dir + "D_" + self.v + ".pkl")

    def load_model(self):
        if os.path.exists(self.save_dir + "G_" + self.v + ".pkl") and os.path.exists(
            self.save_dir + "D_" + self.v + ".pkl"
        ):

            self.generator.load_state_dict(
                torch.load(self.save_dir + "G_" + self.v + ".pkl", map_location={"cuda:1": "cuda:0"})
            )
            self.discriminator.load_state_dict(
                torch.load(self.save_dir + "D_" + self.v + ".pkl", map_location={"cuda:1": "cuda:0"})
            )
            return True
        else:
            return False
